...
class UbahPemeriksaanController extends GetxController {
  ...
  void fetchData(String documentId) async {
    CollectionReference pemeriksaan = FirebaseFirestore.instance.collection('pemeriksaan');
    DocumentSnapshot docSnapshot = await pemeriksaan.doc(documentId).get();

    if (docSnapshot.exists) {
      originalData = docSnapshot.data() as Map<String, dynamic>; // Simpan data awal
  ...
  void loadData(Map<String, dynamic> data) {
    ...
    tanggalLahirController.value = (data['tanggal_lahir'] as Timestamp).toDate();
    jenisKelaminController.value = data['jenis_kelamin'];
    tanggalWaktuPemeriksaanController.value = (data['tanggal_waktu_pemeriksaan'] as Timestamp).toDate();
    ...
    riwayatpenyakitsebelumnyaController.text = data['riwayat_penyakit_sebelumnya'];
  ...
  if (!RegExp(r"^\d{1,3}/\d{1,3}$").hasMatch(tekananDarahController.text)) {
    Get.snackbar('Error', 'Tekanan darah harus berformat angka/angka (misal 120/80)');
    return false;
  }
  ...
  if (!RegExp(r"^\d+(\.\d+)?$").hasMatch(suhuTubuhController.text)) {
    Get.snackbar('Error', 'Suhu tubuh hanya boleh berisi angka dan boleh menggunakan desimal');
    return false;
  }
  ...
  if (!RegExp(r"^\d+(\.\d+)?$").hasMatch(tinggiBadanController.text)) {
    Get.snackbar('Error', 'Tinggi badan hanya boleh berisi angka dan boleh menggunakan desimal');
    return false;
  }
  ...
  if (!RegExp(r"^\d+(\.\d+)?$").hasMatch(beratBadanController.text)) {
    Get.snackbar('Error', 'Berat badan hanya boleh berisi angka dan boleh menggunakan desimal');
    return false;
  }
  ...
  void updateData() async {
    ...
    await pemeriksaan.doc(documentId).update({
      ...
      'tanggal_waktu_pemeriksaan': Timestamp.fromDate(tanggalWaktuPemeriksaanController.value!),
